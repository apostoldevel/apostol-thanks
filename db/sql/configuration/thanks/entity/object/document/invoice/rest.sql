...
--------------------------------------------------------------------------------
/**
 * Запрос данных в формате REST JSON API (Счёт-заказ).
...
BEGIN
  ...
  WHEN '/invoice/method' THEN
    ...
    IF jsonb_typeof(pPayload) = 'array' THEN
      ...
      LOOP
        FOR e IN SELECT r.id, api.get_methods(GetObjectClass(r.id), GetObjectState(r.id)) as method FROM api.get_invoice(r.id) ORDER BY id
        LOOP
          RETURN NEXT row_to_json(e);
        END LOOP;
      END LOOP;
